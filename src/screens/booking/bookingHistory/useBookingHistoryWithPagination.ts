...
const PAGE_SIZE = 300;
const useBookingHistoryWithPagination = (
  ...
  useEffect(() => {
    ...
    // Register the event listener
    const reviewThePro = EventRegister.addEventListener('reviewPro', () => {
      // __DEV__ && Alert.alert(`reviewed pro`);
      fetchInitialData();
    });
    ...
    // Ensure the listener is a string
    if (typeof reviewThePro === 'string') {
      // Cleanup the event listener when the component unmounts
      return () => {
        EventRegister.removeEventListener(reviewThePro);
      };
    }
  }, [
    orderByChild,
  ...
  const fetchInitialData = async () => {
    ...
    const filteredData = dataArray?.filter((item: TBookingDetails) =>
      statusValue?.includes(item?.clientUserUId_status!),
    );
...
export default useBookingHistoryWithPagination;
